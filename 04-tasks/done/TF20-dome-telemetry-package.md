...
**Note (2026-08-04)**: first on-robot attempt failed — `dome_telemetry`'s
`setup.py` `data_files` never installed the `launch/` directory, so
`bl.include` couldn't find `robot.launch.py` in the installed package. Fixed
by adding the `launch/*.launch.py` glob to `data_files` (matching the
`dome_mission`/`dome_nav` convention) and rebuilding. Second attempt hit a
stale `dome_telemetry_msgs` install missing `BatteryStats`; fixed with a
clean rebuild. Re-run on 2026-08-04 succeeded on the live robot.
